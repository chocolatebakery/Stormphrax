...
#define SP_TUNABLE_ASSERTS(Default, Min, Max, Step) \
	...
	SP_TUNABLE_PARAM(incrementScale, 85, 50, 100, 5)
	SP_TUNABLE_PARAM(softTimeScale, 70, 50, 100, 5)
	SP_TUNABLE_PARAM(hardTimeScale, 43, 20, 100, 5)

	SP_TUNABLE_PARAM(nodeTimeBase, 147, 100, 250, 10)
	SP_TUNABLE_PARAM(nodeTimeScale, 185, 100, 250, 10)
	SP_TUNABLE_PARAM(nodeTimeScaleMin, 71, 1, 1000, 100)

	SP_TUNABLE_PARAM(timeScaleMin, 90, 1, 1000, 100)

	SP_TUNABLE_PARAM(minAspDepth, 8, 1, 10, 1)

	SP_TUNABLE_PARAM(maxAspReduction, 5, 0, 5, 1)

	SP_TUNABLE_PARAM(initialAspWindow, 14, 8, 50, 4)
	SP_TUNABLE_PARAM(maxAspWindow, 434, 100, 1000, 100)
	...
	SP_TUNABLE_PARAM(nmpReductionDepthScale, 8, 1, 8, 1)
	SP_TUNABLE_PARAM(nmpReductionEvalScale, 217, 50, 300, 25)
	...
	SP_TUNABLE_PARAM(rfpMarginNonImproving, 73, 25, 150, 5)
	SP_TUNABLE_PARAM(rfpMarginImproving, 32, 25, 150, 5)
	SP_TUNABLE_PARAM(rfpHistoryMargin, 170, 64, 1024, 64)

	SP_TUNABLE_PARAM(maxSeePruningDepth, 4, 4, 15, 1)

	SP_TUNABLE_PARAM(quietSeeThreshold, -73, -120, -20, 10)
	SP_TUNABLE_PARAM(noisySeeThreshold, -89, -120, -20, 10)

	SP_TUNABLE_PARAM(minSingularityDepth, 9, 4, 12, 0.5)

	SP_TUNABLE_PARAM(singularityDepthMargin, 2, 1, 4, 1)
	SP_TUNABLE_PARAM(singularityDepthScale, 12, 8, 32, 2)

	SP_TUNABLE_PARAM(doubleExtensionMargin, 13, 2, 30, 2)
	SP_TUNABLE_PARAM(tripleExtensionMargin, 121, 50, 300, 15)
	SP_TUNABLE_PARAM(multiExtensionLimit, 12, 4, 16, 1)

	SP_TUNABLE_PARAM(maxFpDepth, 8, 4, 12, 0.5)

	SP_TUNABLE_PARAM(fpMargin, 247, 120, 350, 15)
	SP_TUNABLE_PARAM(fpScale, 79, 40, 80, 5)
	...
	SP_TUNABLE_PARAM(maxHistory, 15938, 8192, 32768, 256)

	SP_TUNABLE_PARAM(maxHistoryBonus, 2713, 1024, 3072, 256)
	SP_TUNABLE_PARAM(historyBonusDepthScale, 255, 128, 512, 32)
	SP_TUNABLE_PARAM(historyBonusOffset, 483, 128, 768, 64)

	SP_TUNABLE_PARAM(maxHistoryPenalty, 1462, 1024, 3072, 256)
	SP_TUNABLE_PARAM(historyPenaltyDepthScale, 362, 128, 512, 32)
	SP_TUNABLE_PARAM(historyPenaltyOffset, 158, 128, 768, 64)

	SP_TUNABLE_PARAM(historyLmrDivisor, 7429, 4096, 16384, 512)

	SP_TUNABLE_PARAM(evalDeltaLmrDiv, 455, 100, 1000, 50)
	SP_TUNABLE_PARAM(maxEvalDeltaReduction, 1, 1, 4, 1)

	SP_TUNABLE_PARAM(lmrDeeperBase, 90, 32, 96, 8)
	SP_TUNABLE_PARAM(lmrDeeperScale, 10, 2, 12, 1)

	SP_TUNABLE_PARAM_CALLBACK(lmrBase, 67, 50, 120, 5, updateLmrTable)
	SP_TUNABLE_PARAM_CALLBACK(lmrDivisor, 214, 100, 300, 10, updateLmrTable)

	SP_TUNABLE_PARAM(qsearchFpMargin, 156, 50, 400, 10);
